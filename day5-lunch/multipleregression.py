...
#R-squared=says how much of your variables are predicted in the predictor variable
#the negative coeff (which would be like the slope of the variable) so it means that the correlation diminishes with the sex
#P-value = is the significance, if they are related in some way - the probability that the results 
#the NO hyphotesis is that the coeff is 0(so the values do not depend on each other)
